...
function renderBookmarks(sections: BookmarkSection[], container: HTMLElement) {
  if (sections.length === 0) {
    container.innerHTML =
      '<p class="view-empty view-chip view-chip--dashed">没有匹配的书签。试试调整文件夹筛选，或安装扩展同步浏览器书签。</p>';
    return;
  }
    ...
    <section class="view-bookmark-section" data-folder-id="${section.folderId}">
      ...
      <div class="view-bookmark-grid">
        ${section.links
          .map(
            (link) => `
          <a class="view-bookmark-card view-chip" href="${escapeAttr(link.url)}" target="_blank" rel="noopener noreferrer">
